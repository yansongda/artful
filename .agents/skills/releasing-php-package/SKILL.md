...
**本仓库升级指南路径固定为 `web/docs/v1/upgrade/v{version}.md`**（VitePress 文档站，中文撰写）：
- `upgrade/` 目录不存在时创建（首个升级指南，如 `web/docs/v1/upgrade/v1.2.0.md`）
- 目标版本指南已存在（如后续 beta 版本）时，在原有文件上追加新内容，不要整篇替换

**章节结构：**
1. `## 重点检查`
   - `### 运行环境` — PHP 版本、扩展、其他运行时要求
   - `### 简单使用者` — **仅 end-user 可见变更**（配置项变化、契约方法变化、事件变化等）。不要罗列内部重构（内部辅助函数重组、不影响公共 API 的基类变化、代码组织调整）。
   - `### 自有插件开发者` — 影响自定义 Plugin / Packer / Direction / Shortcut 或容器扩展开发的变更（新契约、方法签名变化、移除的内部工具）。
2. `## 更改版本号` — `composer require yansongda/artful:~{version}`
...
在 `web/.vitepress/config/sidebar.ts` 中将新升级指南注册到侧边栏（新增 upgrade 分组或追加条目；如需顶部入口，同步调整 `web/.vitepress/config/nav.ts`）。

**注意：** `web/docs/` 未被 `.gitignore` 忽略，无需 `git add -f`；改动后建议本地 `pnpm web:dev` 验证页面渲染与导航跳转（Node 环境准备见 container-dev skill）。
...
| PR 未合并就创建 tag | 急于完成发布 | 先确认 `origin/main` 包含合并提交 |
